...
public class BackgroundGeolocationPlugin: CAPPlugin, CAPBridgedPlugin, LocationProviderDelegate, DrivingEventsDetectorDelegate {
    ...
    private static let pluginVersion = "2.1.0"
    ...
    @objc func getCurrentLocation(_ call: CAPPluginCall) {
        guard let facade = facade else { call.reject("facade not initialized"); return }
        // Bounded default (was Int32.max ≈ 24 days): a dropped wait must not park a thread forever.
    ...
    @objc func getLastLocation(_ call: CAPPluginCall) {
        // Sticky replay source: the last fix the plugin observed (or nothing yet).
        if let loc = lastBGLocation {
            call.resolve(loc.toDictionaryWithId())
        } else {
            call.resolve()
        }
    }
