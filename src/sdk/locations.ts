...
export interface LocationsApi {
   ...
   */
  current(options?: CurrentLocationOptions): Promise<Location>;
   ...
   */
  pending(options?: PendingQuery): Promise<Location[]>;
...
export function LocationsApi(deps: { native: BackgroundGeolocationNative }): LocationsApi {
  const { native } = deps;
  let requestSeq = 0; // monotonic id source for correlating current() with its cancel
  ...
  return {
    ...
    async current(options: CurrentLocationOptions = {}) {
      ...
      if (signal === undefined) {
        return native.getCurrentLocation(nativeOptions);
      }
      if (signal.aborted) {
        throw abortError();
      }
      ...
      const requestId = `loc-${++requestSeq}`;
      const request = native.getCurrentLocation({ ...nativeOptions, requestId });
      const onAbort = (): void => {
        void native.cancelCurrentLocation({ requestId });
      };
    ...
    async all() {
      return (await native.getLocations()).locations;
    },
    ...
    stationary() {
      return native.getStationaryLocation();
    },

    delete(locationId) {
      return native.deleteLocation({ locationId });
    },

    clear() {
      return native.deleteAllLocations();
    },
  };
}
